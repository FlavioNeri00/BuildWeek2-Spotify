...
const search = async () => {
  ...
  <div class="grey-cards shadow p-3 pb-5 rounded mt-4">
  ...
  for (let i = 0; i < 4; i++) {
    ...
    <img style="width: 50px" class="me-3 shadow" src="${srcArr[i].album.cover_small}"/>
    <div>
    <p class="m-0 truncate">${srcArr[i].title}</p>
    <a href=./artist.html?ID=${srcArr[i].artist.id} class="text-decoration-none text-white">
    <p class="info m-0 text-secondary ">${srcArr[i].artist.name}</p>
    </a>
     </div>
     </div>
     <p class="info opacity-50 me-5 fw-light">${minTrack}</p>
     </div>`;
  ...
  for (let i = 0; i < 5; i++) {
    ...
    <div class="card grey-cards p-2" style="min-height:300px">
    <img src="${srcArr[i].album.cover}" class="card-img shadow" alt="...">
